...
def fetch_hacker_news_stories(
    ...
    max_item_id = requests.get(f"{base_url}/maxitem.json").json()
    ...
    for item_id in range(max_item_id, 0, -store_every):
        ...
        story_id = item_id
        story_data = item_data

        # Try fetching and extract text from the URL. If HTTPError is raised, skip.
        try:
            story_data['text'] = (
                f"Story Title: {story_data['title']} -- Story text: {extract_text_from_url(story_data['url'])}"
            )
        except HTTPError:
